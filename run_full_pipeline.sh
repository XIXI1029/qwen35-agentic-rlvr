#!/usr/bin/env bash
# =====================================================================
# run_full_pipeline.sh —— 服务器【完整流程】(可断点续跑)
#
...
#     compare         每次都会重跑(秒级)，把已有结果汇总成表
#   ⚠️ 注意：SFT/GRPO 的 checkpoint 只在训练【结束】才落盘；如果中断发生在
#      训练中途，该步没有成品会被【整步重跑】（无法从中间续），但前面已完成的
#      大步不会浪费。
#
# 【控制】
...
EVAL_N="${EVAL_N:-50}"
FORCE="${FORCE:-0}"
# GRPO 配置：默认本机 12GB 版；服务器大显存用 configs/grpo_config.server.yaml
...
# 评估是否需要重跑：结果文件不存在，或【比模型权重旧】（模型更新过就要重评）
...
# ---------- [4/6] 4B GRPO 训练 ----------
# 训练判断：没有成品模型，或【最新 checkpoint 步数 < 目标步数】(说明没训够) -> 继续训
